...
class GreekTokenizer(RomanTokenizer):
    def __init__(self, lang='hy', split_sen=False, smt=False):
        ...
        self.greek_alpha = ''.join([unichr(x) for x in range(0x0370, 0x03ff) if unichr(x).isalpha()])
        self.greek_alpha += ''.join([unichr(x) for x in range(0x1f00, 0x1fff) if unichr(x).isalpha()])
